...
let all = [];
const allNames = data.filter(function (el) {
  return all.push(el.name);
});
console.log(all);

// Question 2: Extract to an array all the job positions for all the people.
let jobs = [];
const allJobs = data.filter(function (el) {
  return jobs.push(el.jobs);
});
console.log(jobs);
...
const allage = data.filter(function (el) {
  return age.push(el.age);
});
